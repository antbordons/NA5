...
@RequiresFullAccess
public class NA5_bi_tempo51 implements IStrategy {
...
// detecta paso por pivot point
   ...
   double pivot_p,pivot_r2,pivot_s2;     
    ...
    arranque[i]=0; 
    print (strategylabel+":"+instrument +"-inicio "+" Estado compra="+estado_compra[i]+" Estado venta="+estado_venta[i]+" Precio="+close.getBid()+" Banda Up="+round(kcup[i],5)+" Banda Down="+round(kcdo[i],5));       
...
//      print(instrument+" Inferior a:"+round(minimo,5));
    ...
    protected int positionsTotal(Instrument instrument, int tipo) throws JFException {
        ...
        for (IOrder order : engine.getOrders(instrument)) 
            ...
            //print("label"+label_estrategia+" y la estrategia label"+strategylabel+" la comparacion="+label_estrategia.compareTo(strategylabel));
       ...
       if (estado_compra[i] == 2)
         ...
         if (permite_entrada[i]==false)
             ...
             IOrder order = engine.submitOrder(getLabel(instrument),instrument,IEngine.OrderCommand.BUY, Lots, 0,Slippage,
                 ...
                 //askbar.getClose() - Point[i] * 1000,askbar.getClose() + Point[i] * TakeProfit[i]);
             horas_compra[i]=1;    
             }
          }       
        // Estado 4 Venta
...
//                    }  
         ...
         if (permite_entrada[i]==false)
             ...
             0, bidbar.getClose() - Point[i] * TakeProfit[i]);
             //bidbar.getClose() + Point[i] * 1000, bidbar.getClose() - Point[i] * TakeProfit[i]);
             horas_venta[i]=1;           
             }
          } 
