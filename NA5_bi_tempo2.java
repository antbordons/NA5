...
public class NA5_bi_tempo2 implements IStrategy {
   ...
   private double piv_P, piv_R2, piv_S2;
   private int pivot_revasado=0;
   private boolean permite_entrada=true;
...
// detecta paso por pivot point
    if (tick.getBid()> piv_R2 || tick.getBid()<piv_S2)
        {
        pivot_revasado=1;
...
if(i != 0  && period==Period.DAILY)
   ...
   int dia_semana= calendar.get(Calendar.DAY_OF_WEEK);
    ...
    if (dia_semana >1 && dia_semana <=6)
        ...
        piv_R2= piv_P +piv_high-piv_low;
        piv_S2= piv_P -piv_high+piv_low;
        
    
        print("mes="+mes+" dia="+dia+"pivots="+piv_P+" y "+piv_R2);
