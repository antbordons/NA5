...
public class NA5_tempo implements IStrategy {
  ...
  // @Configurable("Spread maximo") public double max_spread= 2.0;
    ...
    protected void parametros_iniciales()
    {
        ...
        if(estadoeu==0)
            ...
            arranque[1]=1;
            estado[1]=1;
            estado[1]=1; //cambio 3-agosto-2011
            }
        else
            {
            arranque[1]=0;
...
//    calculos(instrument,i,tick);
   ...
   // busca la tendencia: 
       ...
       do{               
            n++;
           will= indicators.willr(instrument, periodo, OfferSide.BID, PeriodoWill[i],Filter.ALL_FLATS,n,close.getTime(), 0)[0];   
   ...
   //if (n == normaliza_hora(dia,dia_semana,mes)) 
       double preu=history.getBar(instrument,periodo,OfferSide.BID,resultat).getClose();
...
if(i != 0  && period==periodo)
   ...
   if (abierto(hora,dia_semana)==1) 
      ...
      print (instrument +"-h "+" Estado="+estado[i]+" Precio="+bidbar.getClose()+" Banda Up="+round(kcup[i],5)+" Banda Down="+round(kcdo[i],5));
      }    
   }   
}
