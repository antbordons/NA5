...
public class NA501 implements IStrategy {
  ...
  // @Configurable("Spread maximo") public double max_spread= 2.0;
    ...
    @Configurable ("Take profit") public int p_takeprofit=800;    
    ...
    @Configurable ("N. totales ") public int numero_totales=3;
    @Configurable ("factor multiplicacion ") public double multiplica=1.5;
...
//+----------------------------------------------------            
    ...
    if (estado_old[i] != estado[i])
        ...
        print (instrument +"-cambio "+" Estado="+estado[i]+" Precio="+tick.getBid()+" Banda Up="+round(kcup[i],5)+" Banda Down="+round(kcdo[i],5));
        }
//    calculos(instrument,i,tick);
   ...
   //if (n == normaliza_hora(dia,dia_semana,mes)) 
    ...
    arranque[i]=0; 
    print (instrument +"-inicio "+" Estado="+estado[i]+" Precio="+close.getBid()+" Banda Up="+round(kcup[i],5)+" Banda Down="+round(kcdo[i],5));       
...
//    print("lotes "+Lots+" y sin redondeo="+ Lots2);
    if (totales>=numero_totales)
        {Lots=Lots*multiplica;}
    return Lots;
    }    
...
if(i != 0  && period==Period.ONE_HOUR)
   ...
   if (n == normaliza_hora(dia,dia_semana,mes) && abierto(hora,dia_semana)==1) 
      ...
      print (instrument +"-4h "+" Estado="+estado[i]+" Precio="+bidbar.getClose()+" Banda Up="+round(kcup[i],5)+" Banda Down="+round(kcdo[i],5)+" totales="+totales);
      }    
   }   
}
	...
	public void onMessage(IMessage message) throws JFException {
        ...
        if(message.getType()==IMessage.Type.ORDER_FILL_OK)
            ...
            if (estado[im]==6) estado[im]=4;
            if (estado[im]==5) estado[im]=2;
