...
@RequiresFullAccess
public class NA5_bi_tempo52 implements IStrategy {
    ...
    private int []cortebb_up = new int [11];
    private int []cortebb_do = new int [11];
    private int []cortebb_me = new int [11];
    ...
    private double []take_venta = new double[11];
    private double []take_compra = new double[11]; 
    private String []nombrefichero = new String[11]; 
    private int []horas_compra =  new int[11];
    private int []horas_venta =  new int[11];
    private int []trades = new int[11];
    private double []profit_inst = new double[11];
    private double []pipsprofit_inst = new double[11];
  ...
  // @Configurable("Spread maximo") public double max_spread= 2.0;
    ...
    @Configurable ("bollinger ") public int p_boll=20;
    @Configurable ("Starc ") public int p_starc=25;
    @Configurable ("Atr ") public double p_atr=3;
    ...
    @Configurable ("rango de recuperacion") public double rango=450;
    //@Configurable ("porcentaje") public double porcentaje=1.02;
    @Configurable ("maximas horas") public int maximo_horas=15;
...
//+----------------------------------------------------            
    ...
    public void onStop() throws JFException {
       ...
       print ("profit en Pip: " + profit_ppips);
        ...
        for(int l=1;l<11;l++)
            {
            print("instrumento"+l+" trades="+trades[l]+" profit="+profit_inst[l]+" pips="+pipsprofit_inst[l]);    
            }
    }
...
/*    if(DebugMode==true) calendar.setTimeInMillis(tick.getTime());  // esto solo es necesario  cuando estamos en backtesting..
    ...
    if (estado_old[i] != estado_compra[i])
        ...
        print (instrument +"-cambio compra hora"+calendar.get(Calendar.HOUR_OF_DAY)+" dia"+calendar.get(Calendar.DAY_OF_MONTH)+"min "+calendar.get(Calendar.MINUTE)+" Estado compra="+estado_compra[i]+" Precio="+tick.getBid()+" Banda Up="+round(kcup[i],5)+" permite"+permite_entrada[i]);
        }
...
/*    if (estado_compra[i] ==1 || estado_compra[i]== 5) // busca compra
    ...
    //            orden_compra.setTakeProfitPrice(tick.getBid() + Point[i] * TakeProfit[i]);  
                //estado_compra[i]=9;  
                }
            }
         }   
    ...
    if (estado_venta[i] ==3 || estado_venta[i]== 6)
       ...
       calculos_tick(instrument,i,tick);  
        ...
        else if (tick.getBid() > stop_venta[i])
            {     
    ...
    //            orden_venta.setTakeProfitPrice(tick.getBid() - Point[i] * TakeProfit[i]);
                }
            }
        // si llega al stop
...
//     }
   ...
   double pivot_p,pivot_r2,pivot_s2;     
    ...
    public void calculos_five(Instrument instrument, int j, IBar close) throws JFException
    {
   ...
   kcdo[j]=ma;
    ...
    double bolinger= indicators.bbands (instrument, periodo, OfferSide.BID, IIndicators.AppliedPrice.MEDIAN_PRICE, p_boll,p_atr,p_atr,
                        IIndicators.MaType.SMA,Filter.ALL_FLATS,1,close.getTime(),0)[0][0]; // alto 
    double bolinger1= indicators.bbands (instrument, periodo, OfferSide.BID, IIndicators.AppliedPrice.MEDIAN_PRICE, p_boll,p_atr,p_atr,
                       IIndicators.MaType.SMA,Filter.ALL_FLATS,1,close.getTime(),0)[1][0]; // medio
    double bolinger2= indicators.bbands (instrument, periodo, OfferSide.BID, IIndicators.AppliedPrice.MEDIAN_PRICE, p_boll,p_atr,p_atr,
                        IIndicators.MaType.SMA,Filter.ALL_FLATS,1,close.getTime(),0)[2][0]; // bajo
    cortebb_up[j]=0;
    cortebb_do[j]=0;
    cortebb_me[j]=0;
    if (close.getClose()>bolinger) cortebb_up[j]=1;
    if (close.getClose()<bolinger2) cortebb_do[j]=1;
    if (close.getClose()<bolinger1) cortebb_me[j]=2; // debajo
    else cortebb_me[j]=1; //arriba
      
     
    corte_up[j]=0;
    corte_do[j]=0;
    //corte_medio=0;
    if (close.getHigh()>kcup[j]) corte_up[j]=1;
    if (close.getLow()<kcdo[j]) corte_do[j]=1;
    }
 ...
 //   print(calendar.get(Calendar.DAY_OF_MONTH)+"/"+calendar.get(Calendar.MONTH)+"/"+calendar.get(Calendar.YEAR)+"--"+calendar.get(Calendar.HOUR_OF_DAY)+":"+calendar.get(Calendar.MINUTE));
   // print("precio="+close.getBid()+" bollinger 0 ="+bolinger+" boll 1 ="+bolinger1+" media ="+ma);
    corte_up[j]=0;
    corte_do[j]=0;
    giroup[j]=0;
    girodo[j]=0;
...
//      print(instrument+" Inferior a:"+round(minimo,5));
    if (close.getBid()>bolinger) cortebb_up[j]=1;
    if (close.getBid()<bolinger2) cortebb_do[j]=1;
    if (close.getBid()<bolinger1) cortebb_me[j]=2; // debajo
    else cortebb_me[j]=1; //arriba
...
if(i != 0  && period==periodo)
   ...
   //if (abierto(hora,dia_semana)==1 && (diade==0 || diade==dia_semana)) 
       ...
       if (estado_compra[i] == 2  || estado_compra[i]==7)
         {
//         if (cortebb_do[i]==1){estado_compra[i]=7;}
//         else
         horas_compra[i]++;
         if(horas_compra[i]>=maximo_horas)
             {
             if (cortebb_do[i]==1){estado_compra[i]=7;}
              //cierra compra    
              IOrder orden_c = orden(instrument,1);
             //if((orden(instrument,1)!= null)  && orden(instrument,1).getOpenPrice()< bidbar.getClose())
             if(orden_c!= null)  
                {
                //orden(instrument,1).close();
                //estado_compra[i]=1;
                double tp= orden_c.getTakeProfitPrice();
                double tpactual= round(bidbar.getClose() + Point[i] * rango,5);
                if (tp != tpactual)
                    {
                    orden_c.setTakeProfitPrice(tpactual); 
                    //print("tp="+tp+" y actual="+tpactual);
                    } 
                //orden(instrument,1).setTakeProfitPrice(bidbar.getClose() + Point[i] * rango);
                //estado_compra[i]=7;
         ...
         if (permite_entrada[i]==false)
             ...
             take_compra[i]=bidbar.getClose()+ Point[i]* TakeProfit[i];
        ...
        // Estado 4 Venta
       //if (estado_venta[i] == 4)
       if (estado_venta[i] == 4 || estado_venta[i]==8)
         {
         //if(cortebb_up[i]==1){estado_venta[i]=8;}   
         //else 
         horas_venta[i]++;
         if(horas_venta[i]>=maximo_horas)
             {
             if(cortebb_up[i]==1){estado_venta[i]=8;}   
                  
              IOrder orden_v = orden(instrument,2);
             if(orden_v!= null)
                {
                double tpv= orden_v.getTakeProfitPrice();
                double tpactualv= round(bidbar.getClose() - Point[i] * rango,5);
                if (tpv != tpactualv)
                    orden_v.setTakeProfitPrice(tpactualv);  
                    
//                    orden(instrument,2).setTakeProfitPrice(bidbar.getClose() - Point[i] * rango);
                    //estado_venta[i]=8;
//                    }  
                /*if(permite_entrada[i]==false)
                    {
         ...
         if (permite_entrada[i]==false)
             ...
             take_venta[i]= bidbar.getClose()- Point[i] * TakeProfit[i];
...
if(i != 0  && period==Period.FIVE_MINS)
   ...
   if(DebugMode==true) calendar.setTimeInMillis(history.getLastTick(instrument).getTime());  // esto solo es necesario  cuando estamos en backtesting..
    ...
    if (estado_compra[i] ==1 || estado_compra[i]== 5) // busca compra
       ...
       calculos_five(instrument,i,bidbar);
        if (arranque[i]==1)
        ...
        if (estado_compra[i]==1 && corte_do[i]==1){estado_compra[i]=5;
         ...
         {
         if (positionsTotal(instrument,COMPRA) ==0) {estado_compra[i]=1;}
         else if (estado_compra[i] == 7 && cortebb_me[i]==1)
             {
             IOrder orden_c= orden(instrument,1);
             if (orden_c != null)
                {
...
/*        else 
    ...
    if (estado_venta[i] ==3 || estado_venta[i]== 6)
       ...
       calculos_five(instrument,i,bidbar);  
        ...
        {
        if (positionsTotal(instrument,VENTA) == 0) {estado_venta[i]=3;}
         else if (estado_venta[i] == 8 && cortebb_me[i]==2)
             {
            IOrder orden_v= orden(instrument,2);
            if (orden_v != null)
                {
...
/*        else if (tick.getBid() > stop_venta[i])
	...
	public void onMessage(IMessage message) throws JFException {
        ...
        if(message.getType()==IMessage.Type.ORDER_CLOSE_OK)
            {
            int is=indice(message.getOrder().getInstrument());
           ...
           gananciames=gananciames+profit_operation;
            trades[is]++;
            profit_inst[is]= profit_inst[is]+profit_operation; 
            pipsprofit_inst[is]= pipsprofit_inst[is]+profit_poperation;
