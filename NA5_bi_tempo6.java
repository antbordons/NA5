...
@RequiresFullAccess
public class NA5_bi_tempo6 implements IStrategy {
  ...
  // @Configurable("Spread maximo") public double max_spread= 2.0;
    ...
    @Configurable ("Debug Mode") public boolean DebugMode=true;
    ...
    for(int l=1;l<11;l++)
        ...
        estado_venta[l]=3;
        estado_compra[l]=1;
        StopLoss[l]=p_stoploss;
        TakeProfit[l]=p_takeprofit;
        }     
...
/*    if (estado_old[i] != estado_compra[i])
    ...
    if (estado_compra[i] ==1) // busca compra
         {     
...
if(i != 0  && period==periodo)
   ...
   //if (abierto(hora,dia_semana)==1 && (diade==0 || diade==dia_semana)) 
       ...
       //print("open="+bidbar.getOpen()+" y close="+bidbar.getClose());                  
       ...
       if (estado_compra[i] == 2)
         ...
         if(horas_compra[i]>=maximo_horas)
             ...
             if(orden_c!= null)  
                ...
                double tpactual= round(bidbar.getClose() + Point[i] * TakeProfit[i],5);
                if (bidbar.getOpen()>bidbar.getClose())
                        tpactual= round(bidbar.getClose() + Point[i] * StopLoss[i],5);
                if (tp != tpactual)
                    {
       ...
       if (estado_venta[i] == 4)
         ...
         if(horas_venta[i]>=maximo_horas)
             ...
             if(orden_v!= null)
                ...
                double tpactualv= round(bidbar.getClose() - Point[i] * TakeProfit[i],5);
                if (bidbar.getOpen()<bidbar.getClose())
                        tpactualv= round(bidbar.getClose() - Point[i] * StopLoss[i],5);
                if (tpv != tpactualv)
	...
	public void onMessage(IMessage message) throws JFException {
        ...
        if(message.getType()==IMessage.Type.ORDER_CLOSE_OK)
            ...
            if (estado_venta[is]==4 && message.getOrder().getOrderCommand()==IEngine.OrderCommand.SELL) 
                {
                estado_venta[is]=3;
                }
            if (estado_compra[is]==2 && message.getOrder().getOrderCommand()==IEngine.OrderCommand.BUY) 
                {
                estado_compra[is]=1;
                }
