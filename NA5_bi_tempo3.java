...
public class NA5_bi_tempo3 implements IStrategy {
    ...
    private int []posicion = new int[11];
  ...
  // @Configurable("Spread maximo") public double max_spread= 2.0;
    ...
    @Configurable ("Profit ") public int p_takeprofit=500;
...
//+----------------------------------------------------            
    ...
    if (estado_old[i] != estado_compra[i])
        ...
        print (instrument +"-cambio compra hora"+calendar.get(Calendar.HOUR_OF_DAY)+" Estado compra="+estado_compra[i]+" Precio="+tick.getBid()+" Banda Up="+round(kcup[i],5)+" Banda Down="+round(kcdo[i],5));
        }
    if (estado_oldv[i] != estado_venta[i])
        {
        estado_oldv[i]=estado_venta[i];
        print (instrument +"-cambio venta hora"+calendar.get(Calendar.HOUR_OF_DAY)+" Estado venta="+estado_venta[i]+" Precio="+tick.getBid()+" Banda Up="+round(kcup[i],5)+" Banda Down="+round(kcdo[i],5));
...
// detecta paso por pivot point
   ...
   double pivot_p,pivot_r2,pivot_s2;     
    ...
    //IBar barradia =history.getBar(instrument, Period.DAILY, OfferSide.BID,1);
    //IBar barradia2 =history.getBar(instrument, Period.DAILY, OfferSide.BID,2);
    /*double pivot_high=barradia2.getHigh();
    double pivot_low=barradia2.getLow();
    double pivot_close=barradia2.getClose();
    double p_p, p_r2, p_s2;
    p_p= (pivot_high+pivot_low+pivot_close)/3;
    p_r2= p_p+pivot_high-pivot_low;
    p_s2= p_p-pivot_high+pivot_low;
    if (barradia.getHigh()>p_r2 || barradia.getLow() < p_s2)
        {
        permite_entrada[i]=false;    
        }   
    else
